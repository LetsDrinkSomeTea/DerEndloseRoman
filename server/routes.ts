...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/stories/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (rootChapter) {
        const continuationOptions = await storage.getContinuationOptions(
          rootChapter.id,
        );
        res.json({
          ...story,
  ...
  app.post("/api/stories", async (req: Request, res: Response) => {
    try {
      ...
      res.status(201).json({
        ...
        rootChapter: {
          ...rootChapter,
          continuationOptions: await storage.getContinuationOptions(
            rootChapter.id,
          ),
        },
      });
      ...
      res
        .status(500)
        .json({ message: "Fehler beim Erstellen der Geschichte." });
    }
  });
  ...
  app.get("/api/chapters/:id", async (req: Request, res: Response) => {
    try {
      ...
      const continuationOptions =
        await storage.getContinuationOptions(chapterId);
  ...
  app.get("/api/chapters/:id/path", async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Fehler beim Abrufen des Kapitelpfads." });
    }
  });
  ...
  app.post("/api/stories/continue", async (req: Request, res: Response) => {
    try {
      ...
      const { storyId, chapterId, selectedOptionId, customPrompt } =
        validatedData;

      if (!selectedOptionId && !customPrompt) {
        return res.status(400).json({
          message:
            "Es muss entweder eine Fortsetzungsoption oder ein eigener Prompt angegeben werden.",
        });
      }
      ...
      if (selectedOptionId) {
        ...
        if (!option) {
          return res
            .status(404)
            .json({ message: "Fortsetzungsoption nicht gefunden." });
        }

        prompt = option.prompt;

        // Prüfe, ob bereits ein Kapitel für diese Option existiert
        existingChapter = await storage.getNextChapterByOption(
          chapterId,
          selectedOptionId,
        );
      ...
      res
        .status(500)
        .json({ message: "Fehler bei der Fortsetzung der Geschichte." });
    }
  });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!story) {
          return res
            .status(404)
            .json({ message: "Geschichte nicht gefunden." });
        }
        ...
        res
          .status(500)
          .json({ message: "Fehler beim Abrufen der Charaktere." });
      }
    },
  );
